...
sonarAlt = srf08_ctx.range[0] - SONAR_OFFSET; //tmp
...
#elif defined (externLidar) 
//************************************************************************************************************
// Lidar via an external coprocessor. Communication to/from the coprocessor is via i2c
// ************************************************************************************************************

#elif defined(genericLidar)

//************************************************************************************************************
// Lidar via PWM Support
//
// Please note that the the minimum reading in pwm mode is ~30.
// Thus, we have to subtract a rather large offset. The pwm code itself is based
// on work by jaysonragasa (http://www.multiwii.com/forum/viewtopic.php?f=8&t=6282) and
// kataventos (http://www.multiwii.com/forum/viewtopic.php?f=7&t=1033&start=170#p36603)
// ************************************************************************************************************
volatile unsigned long LIDAR_GEP_startTime = 0;
volatile unsigned long LIDAR_GEP_echoTime = 0;
volatile static int32_t  tempLidarAlt = 0;

void Lidar_init() {
   LIDAR_GEP_EchoPin_PCICR;
   LIDAR_GEP_EchoPin_PCMSK;
   LIDAR_GEP_EchoPin_PINMODE_IN;
   LIDAR_GEP_TriggerPin_PINMODE_OUT;
   #if defined (CONTINUOUS_LIDAR)
     LIDAR_GEP_TriggerPin_PIN_LOW; //continuous Measurement
   #endif
}
...
void Lidar_update() {
   ...
   #if defined(LIDAR_TILT_CORRECTION)
            float temp = cos((float(att.angle[1])/10)*0.0174532925f) * cos((float(att.angle[0])/10)*0.0174532925f);           
...
ISR(LIDAR_GEP_EchoPin_PCINT_vect) {
   if (LIDAR_GEP_EchoPin_PIN & (1 << LIDAR_GEP_EchoPin_PCINT)) {
      LIDAR_GEP_startTime = micros();
   }
   else {
      LIDAR_GEP_echoTime = micros() - LIDAR_GEP_startTime;
      
      if (LIDAR_GEP_echoTime <= LIDAR_GENERIC_MAX_RANGE*LIDAR_GENERIC_SCALE)
         tempLidarAlt = LIDAR_GEP_echoTime / LIDAR_GENERIC_SCALE;
      else
         tempLidarAlt = -1;
   }
}
